...
<div id="sideModal_JobDescription" class="side-modal">
    <div class="side-modal-content">
        <div class="side-modal-header">
            ...
            <button type="button" class="btn-close" id="closeSideModalDescription">
            </button>
        </div>
        <div class="side-modal-body">
...
<div id="sideModal" class="side-modal">
    ...
    $target_companies = App\Models\Company::all();
    $user = \Auth::user();
    $companies = collect();
    if ($user->hasRole('Recruiter Company')) {
        ...
        if ($user->company && $user->company->created_by) {
            $creator = App\Models\User::with(['companies'])->find($user->company->created_by);
            $companies = $creator->companies ?? collect();
        }
    }
    ?>
    <div class="side-modal-content">
        <div class="side-modal-header job-form">
            <h5>Add Job</h5>
            <button type="button" class="btn-close" id="closeSideModal"></button>
        </div>
        <div class="side-modal-body">
            <form id="jobForm" class="job-form" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="secondary_contacts" class="form-select select2 select2-multiple"
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" name="job_type" placeholder="Full-time / Contract"
                            required>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" name="job_category" placeholder="e.g., Engineering">
                    </div>
                    <div class="col-md-6 mb-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select id="job_collaborator" class="form-control select2 select2-multiple"
                            multiple="multiple" data-placeholder="Choose" multiple="multiple"
                            name="job_collaborator[]">
                ...
                <div class="text-end">
                    <button type="submit" class="btn btn-success submit-btn">Save Job</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('btn-close')) {
                e.target.closest('.side-modal').classList.remove('show');
            }
        });
    });
    ...
    async function editJob(jobId) {
        try {
            ...
            // ✅ Auto-fill inputs, textareas, and selects
            Object.keys(job).forEach(key => {
                ...
                if (field.tagName === "SELECT") {
                    ...
                    if (typeof $ !== 'undefined' && $.fn.select2) {
                        $(field).val(job[key]).trigger('change');
                    }
                } else if (field.type === "checkbox" || field.type === "radio") {
                    field.checked = !!job[key];
                } else {
                    field.value = job[key] ?? '';
                }
            });
            ...
            //     const collabSelect = form.querySelector('[name="collaborator[]"]');
            //     if (collabSelect) {
            //         const ids = job.collaborators.map(c => c.id);
            //         $(collabSelect).val(ids).trigger('change');
            //     }
            // }
            // console.log(job.collaborator_ids);
            if (collaborator_ids && Array.isArray(collaborator_ids)) {
                const jobcollaboratorSelect = form.querySelector('[name="job_collaborator[]"]');
                if (jobcollaboratorSelect) {
                    if (typeof $ !== 'undefined' && $.fn.select2) {
                        $(jobcollaboratorSelect).val(collaborator_ids).trigger('change');
                    } else {
                        // fallback for plain select without select2
                        [...jobcollaboratorSelect.options].forEach(opt => {
                            opt.selected = collaborator_ids.includes(parseInt(opt.value));
                        });
                    }
                }
            }

            // ✅ Multi-select (secondary contacts)
            if (Array.isArray(contacts)) {
                const contactSelect = form.querySelector('[name="secondary_contacts[]"]');
                if (contactSelect) {
                    if ($ && $.fn.select2) {
                        $(contactSelect).val(contacts).trigger('change');
                    } else {
                        [...contactSelect.options].forEach(opt => {
                            opt.selected = contacts.includes(parseInt(opt.value));
                        });
                    }
                }
            }

            // ✅ Skills (convert string to tags)
            if (job.skills) {
            ...
            if (questions?.length) {
                ...
                questions.forEach((q, i) => {
                    ...
                    div.className = 'question-group mb-2';
                    div.innerHTML = `
                    <div class="input-group">
                        <input type="text" class="form-control" name="questions[]" value="${q}" placeholder="Enter question ${i+1}" required>
                ...
                if (typeof tinymce !== 'undefined') {
                    ...
                    if (editor) {
                        ...
                        tinymce.init({
                            selector: '#classic',
                            setup: ed => {
                                ed.on('init', () => ed.setContent(job.job_description || ''));
                            }
                        });
                    }
                } else {
            ...
            document.querySelector('.job-form h5').textContent = 'Edit Job';
            document.getElementById('sideModal').classList.add('show');
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Event delegation: works for any .job-form, even if multiple exist
        document.addEventListener("submit", async function(e) {
            if (!e.target.classList.contains("job-form")) return;
            ...
            try {
                ...
                try {
                    if (typeof tinymce !== "undefined") {
                        ...
                        if (editor) {
                            ...
                            console.warn("TinyMCE editor not found, using textarea directly");
                        }
                    } else {
                        jobDescription = document.getElementById("classic").value;
                        console.warn("TinyMCE not loaded, using textarea directly");
                    }
                } catch (error) {
                ...
                console.log(url);
                const method = "POST"; // Laravel accepts PUT via _method
                ...
                await Swal.fire({
                    title: "Success!",
                    text: data.message ||
                        (editId ? "Job updated successfully!" :
                ...
                if (err.message.includes("419")) {
                    ...
                    await Swal.fire({
                        title: "Error",
                        text: err.message ||
                            (editId ? "Failed to update job" : "Failed to create job"),
                        icon: "error",
                    });
                }
            } finally {
...
<script>
    $(document).ready(function() {
        ...
        $('select[name="company_id"]').on('change', function() {
            ...
            const secondaryContactSelect = $('#secondary_contacts'); // Changed to use ID selector

...
@section('content')
    ...
    <div class="container py-5">
        ...
        @if ($company_users->isEmpty())
            <div class="text-center py-5">
                <img src="https://cdn-icons-png.flaticon.com/512/4076/4076549.png" alt="No Data" width="100">
                <h5 class="mt-3">You haven't created any company users yet.</h5>
                <p class="text-muted">Start by adding your first company user.</p>
            </div>
        @else
            <div class="row g-4">
                @foreach ($company_users as $user)
                    <div class="col-md-6 col-lg-4">
                        <a href="{{ route('companies.show', $user->id) }}" class="text-decoration-none">
                            <div class="card h-100 shadow-sm border-0 hover-shadow">
                                <div class="card-body">
                                    <h5 class="card-title text-dark hover-zoom text-uppercase">{{ $user->name }}</h5>
                                    <p class="card-text text-muted mb-0">
                                        <i class="bi bi-geo-alt-fill"></i> {{ $user->city ?? 'No City' }}
                                    </p>
                                </div>
